...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($pname) OR empty($pscore)) {
        ...
        if(empty(trim($_POST["pname"]))){
            ...
            $sql = "SELECT id FROM flappybotlb WHERE pname = ?";
            ...
            if($stmt = mysqli_prepare($conn, $sql)){
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    if(mysqli_stmt_num_rows($stmt) == 1){
                        $sql = "UPDATE flappybotlb SET pscore = '$pscore' WHERE pname = '$pname' AND pscore < '$pscore'";
                        if (mysqli_query($conn, $sql)) {
                            header("Location: https://3dxp.co/gaming/unity/web/floppybot/");
                        } else {
                            echo "Error: " . $sql . "<br>" . mysqli_error($conn);
                        }
                    } else{
                        $sql = "INSERT INTO flappybotlb (pname, pscore) VALUES ('$pname', '$pscore')";
                        if (mysqli_query($conn, $sql)) {
                            header("Location: https://3dxp.co/gaming/unity/web/floppybot/");
                        } else {
                            echo "Error: " . $sql . "<br>" . mysqli_error($conn);
        ...
        <div class="row ">
            ...
            <div class="col-xl-6">
                <div class="signup-wrapper signup">
                    <h2 class="mb-2 text-center">You scored:</h2>
                    <br>
                    <h1 class="mb-2 text-center"><?php echo $pscore?></h1>
                    <p class="medium text-center">Enter your name to add your score to Flappy Bot Leaderboards!</p>
                    <br>
